...
public class OwnerView extends JFrame implements ActionListener{
	PropertyController pc;  //use of Aggregation
	ModifyFrame mf;
	HideFrame hf;
	UnHideFrame uhf;
	private String own;
	...
	public OwnerView(String owner){
		...
		modifyProperty = new JButton("Modify Property");	//modifyProperty button
		hide = new JButton("Hide");	//hide property button
		unhide = new JButton("UnHide"); 	//un hide property button
		logOutButton = new JButton("Logout"); // logout button
		uploadPropertyButton = new JButton("Upload Property"); //upload button
        ...
        for(int i = 0; i < LoginFrame.propertyList.size() ; i++ ){
			if(LoginFrame.propertyList.get(i).getHiddentStatus() == false){	// views all property
				if(LoginFrame.propertyList.get(i).getOwner().equals(own)){	// this condition makes to view Only their own Property
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() == logOutButton){
			...
			dispose();
			pc = new PropertyController(own); //Calls propertyController to choose which type of property to upload
			
        }
		else if(e.getSource() == modifyProperty){
			dispose();
			mf = new ModifyFrame(own);	//modifyFrame takes input of the property id that needs to be modified
			
		}
		else if(e.getSource() == hide){
			dispose();
			hf = new HideFrame(own); // takes input of the property id that needs to be hidden from tenant & admin view
			
		}
		else if(e.getSource() == unhide){ 
			dispose();
			uhf = new UnHideFrame(own);// takes input of the property id that needs to be unhidden from tenant & admin view
			...
			switch (cb.getSelectedIndex()){
			   ...
			   case 1:
					dispose();
					OwnerView ov = new OwnerView(own); // calls itself
				break;
				
				case 2:
					dispose();
					OwnerViewActive oav = new OwnerViewActive(own);	// active property by owner
				break;
				case 3:
					dispose();
					OwnerViewInActive ovia = new OwnerViewInActive(own); // Inactive property by owner
				break;
				case 4:
					dispose();
					OwnerViewHide ovh = new OwnerViewHide(own); // hidden property by owner
				break;
			}
		}
